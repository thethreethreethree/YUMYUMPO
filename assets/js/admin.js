  ...
  container.innerHTML = list.map(r => {
    ...
    return `
      <div class="restaurant-row">
        ...
        <div class="flex items-center gap-0.5 sm:gap-1 flex-wrap shrink-0 justify-end max-w-[180px] sm:max-w-none">
...
window.toggleDemo = async function(id) {
  const r = ADMIN_RESTAURANTS.find(x => x.id === id);
  if (!r) return;
  const newVal = !r.is_demo;
  const verb = newVal ? 'mark as demo (hides from public surfaces)' : 'mark as real (will show on /discover and /vibe-map)';
  if (!confirm(`${r.name}: ${verb}?`)) return;

  const client = window.YYP?.client;
  if (client) {
    const { error } = await client.from('restaurants').update({ is_demo: newVal }).eq('id', id);
    if (error) { alert('Update failed: ' + error.message); return; }
  }
  r.is_demo = newVal;
  renderRestaurantList(filteredRestaurants);
