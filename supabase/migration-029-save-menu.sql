-- ============================================================
-- YUMYUMPO — Migration 029 · Menu-only save RPC
-- The dedicated Digital Menu Builder needs to persist just the
-- menu without touching the rest of the restaurant record.
-- save_restaurant_menu() atomically replaces a restaurant's
-- categories + items in one transaction.
-- ============================================================
...
DECLARE
  ...
  v_item_idx INT;
BEGIN
  /* Only the restaurant's owner may save its menu. */
  IF NOT EXISTS (
