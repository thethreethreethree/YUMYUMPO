-- ============================================================
-- YUMYUMPO — Migration 030 · Owner-accessible analytics
-- The redesigned owner dashboard surfaces analytics to the
-- restaurant owner, not just admins. Reopen get_restaurant_
-- analytics so a restaurant's own owner may call it for their
-- restaurant (admins still see every restaurant).
-- ============================================================
...
DECLARE
  ...
  v_recent JSONB;
BEGIN
  /* Admin sees all; an owner sees only their own restaurant. */
  IF NOT (
    is_admin()
    OR EXISTS (
  ...
  FROM (
    SELECT jsonb_build_object(
      ...
      'quoted_total', quoted_total, 'created_at', created_at
    ) AS row
    FROM order_requests
    WHERE restaurant_id = p_restaurant_id
  ...
  v_result := jsonb_build_object(
    ...
    'traffic', jsonb_build_object(
      ...
      'digital_menu_30d',  (SELECT COUNT(*) FROM analytics_events WHERE restaurant_id = p_restaurant_id AND event_type = 'digital_menu_view' AND created_at > NOW() - INTERVAL '30 days'),
      ...
      'instagram_30d',     (SELECT COUNT(*) FROM analytics_events WHERE restaurant_id = p_restaurant_id AND event_type = 'instagram_click' AND created_at > NOW() - INTERVAL '30 days'),
      'messenger_30d',     (SELECT COUNT(*) FROM analytics_events WHERE restaurant_id = p_restaurant_id AND event_type = 'messenger_click' AND created_at > NOW() - INTERVAL '30 days'),
    ...
    'buzz', jsonb_build_object('score', COALESCE(v_buzz.buzz_score, 0), 'tier', v_buzz.buzz_tier),
